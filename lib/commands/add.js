...
function addCommand(args = []) {
  ...
  if (!action) {
    box('Claude AI Automation - Agent Management');
    log('Usage:', 'bright');
    log('   caia add create <name>     Create a new custom agent');
    log('   caia add remove <name>     Remove an agent');
    log('   caia add <name>            Install a community agent\n');
    return;
  }
  ...
  info(`Community agent registry coming soon. Use "caia add create ${action}" to create a custom agent.\n`);
}
...
function createAgent(args, targetDir) {
  ...
  log(`   Add trigger words to CLAUDE.md routing table.\n`);
}

function removeAgent(args, targetDir) {
  const name = args[0];
  if (!name) {
    error('Agent name required. Usage: caia add remove <name>');
    return;
  }
