...
function configCommand(args = []) {
  ...
  box('Claude AI Automation - Configuration');
  ...
  if (args.length === 0) {
    ...
    log('\nUsage:', 'bright');
    log('   caia config set <key> <value>');
    log('   caia config get <key>');
    log('   caia config reset\n');
    return;
  }
  ...
  if (action === 'set' && args.length >= 3) {
    ...
    warn('Usage: caia config [set <key> <value> | get <key> | reset]\n');
  }
}
