...
function showTelemetryHelp() {
  box('Claude AI Automation - Telemetry');
  log('Anonymous, opt-in usage analytics.\n', 'bright');
  log('Usage:', 'bright');
  log('   caia telemetry on        Enable telemetry');
  log('   caia telemetry off       Disable telemetry');
  log('   caia telemetry status    Show current status');
  log('   caia telemetry stats     Show usage statistics');
  log('   caia telemetry reset     Reset collected data\n');
...
function showStats() {
  ...
  if (!stats) {
    ...
    info('Enable with: caia telemetry on\n');
    return;
  }
