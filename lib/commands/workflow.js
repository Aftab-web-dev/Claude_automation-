...
function showWorkflowHelp() {
  box('Claude AI Automation - Workflows');
  log('Workflows define multi-agent pipelines.\n', 'bright');
  log('Usage:', 'bright');
  log('   caia workflow list              List all workflows');
  log('   caia workflow create <name>     Create a new workflow');
  log('   caia workflow show <name>       Show workflow details');
  log('   caia workflow delete <name>     Delete a workflow\n');
...
function listWorkflows() {
  ...
  if (!fileExists(workflowsDir)) {
    ...
    log('   Create one with: caia workflow create <name>\n');
    return;
  }
...
function showWorkflow(name) {
  if (!name) {
    error('Workflow name required. Usage: caia workflow show <name>');
    return;
  }
...
function deleteWorkflow(name) {
  if (!name) {
    error('Workflow name required. Usage: caia workflow delete <name>');
    return;
  }
